			...
			<div class="container">		
				...
				<br><br>					
					...
					<div class="row">
						...
						<p class="text-success font-weight-bold">SELECT JOB TO HIRE FREELANCER</p>
					<?php
						/*-------------- SEND PROPOSAL -------------*/
						
						$username=$_SESSION['hire'];
						$query_hire="SELECT * FROM `jobs` WHERE `h_id`='$username' AND `status`='pending'";
					
						$run_hire=mysqli_query($conn,$query_hire);
						$num_job=mysqli_num_rows($run_hire);
							
						?>
						<form action="send_proposal.php" method="POST">
						<?php
						while ($row = mysqli_fetch_array($run_hire))
						{
						?>
							<input type="radio" name="job_title" value="<?php echo $row['title'];?>" required><?php echo $row['title'];?><br>		
						<?php	
						}
						if($num_job==0)
						{ ?>
							<h4 class="text-danger">Please First Post Job !! After Select Job !!</h4>
					<?php	}
					else
					{ ?>
						<input type="hidden" value="<?php echo $data['w_id']; ?>" name="w_id">	
						
						<center><input type="submit" value="Invlite To Job" class="btn btn-success"></center>
				<?php	}
						?>		
					...
					<div class="text-center float-left">
						<a href="hire_index.php" class="text-success h5"><--Back</a>
					</div>																
				</div>
			</div>			
		</div>
